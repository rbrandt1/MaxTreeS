...
const char * testCaseGV;
const char * filenameGV;
int skipY = 1;
...
void plotDispmap(cv::Mat dispMap, int disparityLevelsResized, bool side) {
	...
	std::string str = "../trainingF/";
	str.append(testCaseGV);
	str.append("/disp0MaxTreeS_s.pfm");
...
void run(int numThreads, const char* filenameLeftImage,
		...
		int dispKernelSize, const char* filenameDisp, const char* testCase,
		const char * filename) {
	...
	cv::imwrite("sobelLeftHor.tiff", sobelLeft3ucharHor);
	cv::imwrite("sobelRightHor.tiff", sobelRight3ucharHor);

	// create max trees

	treeLeftHor = BuildMaxTrees(numThreads, "sobelLeftHor.tiff", treeLeftHor,
			&height, &width, true);
	treeRightHor = BuildMaxTrees(numThreads, "sobelRightHor.tiff", treeRightHor,
			&height, &width, true);
